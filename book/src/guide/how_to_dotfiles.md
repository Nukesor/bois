...
1. Adding a `hosts/cleo/hosts.yml`, which also opts into the `base` trait:
   ```yml
   traits:
     - base
   ```
...
This is done via the `services` configuration in either `host.yml`, `trait.yml` or `dir.yml` and look like this:
