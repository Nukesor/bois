...
Everything inside it (except the `host.yml` and `vars.yml` special files) is configuration that will be deployed for **this host only**.
...
The template generated by `bois init` already created a trait called `base`, and the generated `milo/host.yml` already enables it:
...
1. Adding a `hosts/cleo/host.yml`, which also opts into the `base` trait:
   ```yml
   traits:
     - base
   ```
...
For this, you simply declare them in your `host.yml` or `trait.yml`:
...
Packages declared in a trait's `trait.yml` are installed on every host that opts into that trait.
This is useful for everyday tools you want want on multiple of your machines:

```yml
# traits/base/trait.yml
packages:
  pacman:
    - git
    - vim
...
`bois` also manages system services, which are declared in `host.yml`, `trait.yml` or `dir.yml` configs like this:
