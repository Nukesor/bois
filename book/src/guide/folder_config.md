...
Bois handles such symlinks in target paths as follows:

- If a directory in a target path is a symlink on the system and that directory has **no** declared permissions (`owner`, `group` or `mode`), the symlink is accepted, as long as it points to a directory.
